...
At dispatch the model override is resolved against the provider's model catalog before the worker spawns: a bare id that matches exactly one catalog entry is rewritten to its prefixed id (`deepseek-v4-flash` → `deepseek/deepseek-v4-flash` on Nous/OpenRouter), so a strict aggregator never receives a bare id and answers a content-free 404. A model id that is definitely not in the provider's catalog blocks the task at dispatch time (`blocked`, `needs_input`) with a message listing the exact catalog ids to use, instead of spawning a worker that would burn its run on the same rejection. Providers with no curated catalog (e.g. custom endpoints) pass the override through unchanged.
